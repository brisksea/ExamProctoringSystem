...
class ExamClient:
    ...
    def on_login_success(self, username, api_client, login_data=None):
        ...
        try:
            print(f"[DEBUG][on_login_success] start_url: {self.exam_default_url}")
            ...
            if not browser_started:
                try:
                    ...
                    self.browser_status.set("Chrome 已启动 (受控模式)")
            ...
            if not browser_started:
                try:
                    # 检查是否有Edge
                    if get_edge_version():
                        self.chrome_controller = EdgeController(
                            config_manager=self.config_manager,
                            default_url=chrome_start_url,
                            disable_new_tabs=self.disable_new_tabs
                        )
                        self.chrome_controller.start()
                        self.browser_status.set("Edge 已启动 (受控模式)")
                        self.log("Edge浏览器已启动，插件已被禁用")
                        browser_started = True
                    else:
                        error_msg += "未检测到Edge浏览器; "
                except Exception as e:
                    print(f"尝试启动Edge失败: {e}")
                    error_msg += f"Edge启动失败: {str(e)}; "
                    self.chrome_controller = None

        except Exception as e:
            self.log(f"启动浏览器过程出错: {str(e)}")
            messagebox.showerror("错误", f"启动浏览器过程出错: {str(e)}")

        if not browser_started:
            self.log(f"无法启动任何受控浏览器: {error_msg}")
            messagebox.showerror("错误", f"启动浏览器失败: {error_msg}")
            # 即使失败也创建监控器，避免后续报错，虽然可能无法工作
            # 但最好是这里直接返回或退出？保持原逻辑继续往下走可能会崩
 
        # 延迟导入 AppMonitor 以加快启动速度
        from app_monitor import AppMonitor
        
        # 创建应用监控器
        # 注意：这里 self.chrome_controller 可能是 ChromeController 或 EdgeController 或 None
            ...
            if self.api_client:
                ...
                if not success:
                    ...
                    if hasattr(self.api_client, 'get_connection_stats'):
                        ...
                        if stats['total_attempts'] > 0 and stats['total_attempts'] % 20 == 0:  # 每20次心跳记录一次
                            self.log(f"连接质量统计: 成功率 {stats['success_rate']:.2%}, "
                                   f"平均响应时间 {stats['avg_response_time']:.2f}秒, "
                                   f"超时次数 {stats['timeout_count']}, "
                                   f"连接错误次数 {stats['connection_error_count']}")
            else:
                pass
            # print(f"[DEBUG][heartbeat_loop] after: connected_to_server={self.connected_to_server}")
    ...
    def stop_monitoring(self, fast_exit=False):
        """
        停止监控
        
        Args:
            fast_exit: 如果为 True，快速退出模式，不等待视频上传完成
        """
        ...
        if hasattr(self, 'screen_recorder') and self.screen_recorder:
            if fast_exit:
                # 快速退出模式：后台停止录屏，不等待上传完成
                self.screen_recorder.stop(fast_exit=True)
                self.log("录屏功能已停止（快速退出模式，文件将在后台上传）")
            else:
        ...
        while self.monitoring:
            if pass_second >= delay_min * 60:
                try:
                    ...
                    if err_msg:
                        # 检查是否与上次的违规消息相同
                        print(err_msg, self.last_violation_message)
                        if err_msg != self.last_violation_message:
                            # 捕获屏幕截图
...
def check_time_sync(api_client, log_func):
    ...
    try:
        ...
        if status and server_time_str:
            ...
            if diff > 120:
                # 使用带倒计时的警告窗口
                client = _  # 获取当前的 ExamClient 实例
                if isinstance(client, ExamClient):
                    client.show_warning(
                        "时间不同步",
...
def check_single_instance():
    ...
    import win32api
    try:
        # 使用本地互斥体，避免全局互斥体需要的管理员权限问题
        mutex_name = "Local\\Supervise3_Client_SingleInstance_Mutex_v2"
        ...
        if last_error == 183:
            ...
            try:
                root = tk.Tk()
                root.withdraw()
                messagebox.showwarning("提示", "监控客户端已经在运行中，请勿重复打开。")
                root.destroy()
            except:
                pass
            return False, None
            
        print(f"Successfully acquired single instance mutex: {mutex_name}")
        return True, handle
    except Exception as e:
        print(f"Single instance check failed: {e}")
        return True, None

# 全局变量保存互斥体句柄，确保程序运行期间不被垃圾回收
_app_mutex_handle = None

def main():
    global _app_mutex_handle
    # 检查是否已有实例
    is_single, _app_mutex_handle = check_single_instance()
    if not is_single:
        sys.exit(0)
