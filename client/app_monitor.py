...
class AppMonitor:
    ...
    def __init__(self, config_manager, chrome_controller, exam_client):
        ...
        self._cached_process_name = None
        self._cached_process_exe = None
    ...
    def check_running_apps(self):
        ...
        try:
            ...
            # 优化：如果PID没变，尝试复用缓存的进程信息
            if pid == self._cached_pid and self._cached_process_name:
                process_name = self._cached_process_name
                process_exe = self._cached_process_exe
            else:
                # 获取进程信息
                ...
                self._cached_process_exe = process_exe
    ...
    def _is_system_process(self, process_name):
        ...
        Returns:
            bool: 是否为系统进程
        """
        return process_name.lower() in { proc.lower() for proc in self.system_processes }
